...
Para o manuseio de *strings*, embora o R base ofereça um excelente material, optamos por seguir a orientação tidyverse. Não só para a organização de dados, mas também para a conversão, transformação, extração e substituição de *strings*, o conjunto de pacotes tidyverse oferece uma resposta mais coerente  e sistematizada. O R base é bom, mas apresenta algumas inconsistências. Vejamos um exemplo.

Um comportamento estranho acontece com a função `paste()`. O separador padrão é um espaço em branco, que muito frequentemente é o que queremos usar. No entanto, isso é algo secundário. O que é estranho é quando queremos incluir um argumento de tamanho zero (e.g. `NULL`, `character(0)`). Como `paste()` se comporta nesses casos? Veja abaixo:
...
"Trata de um conjunto de *wrappers* que faz das funções *strings* do R mais consistentes,
...
Para instalar o  `"stringr"` use a função `install.packages()`. Uma vez instalado, carregue para sua pressente sessão com  `library()`:
...
| `str_trim()`   | trims a string                          | _nenhum_      |


Note que todas as funções no pacote `stringr` começam com `"str_"` seguidas por um termo associado à tarefa a ser desempenhada. Por exemplo,  `str_length()` forence o número de caracteres (*length*) em uma *string*. Além disso, algumas funções foram pensadas para oferecer uma alternativa melhor para funções já existentes. Este é o caso de `str_length()` que foi pensada para substituir `nchar()`. 
...
Vamos começar com `str_c()`. Essa função é equivalente a `paste()` mas em vez de usar o espaço em branco como separador padrão, ela usa *string* vazia `""`, que é um separador mais comum quanto estamos _pasting_ *strings*:
...
Os três argumentos na função são: um vetor `string`, um valor `start` indicando a posição do primeiro caractere para dar um *substring* e um valor `end` indicando a posição do último caractere. Seguem um exemplo simples com uma única *string* da qual os caracteres 1 a 5 são estraídos:
...
Uma caracteristica interessante de `str_sub()` é sua abilidade de trabalhar com índices negativos nas posições `start` e `end`. Quando usamos a posição negativa, a função `str_sub()` começa a contar do último caractere: 
...
Da da mesma forma que  `substring()`, nós podemos fornecer a `str_sub()` um conjunto de posições que serão recicladas sobre a *string*. Mais que isso, nós podemos dar  `str_sub()` uma sequência negativa, algo que  `substring()` ignora:
...
A ideia de `str_pad ()` é tomar uma seqüência de caracteres e colocá-la no começo ou no fim da *string* com uma "cumprimento" total especificado. O caractere de preenchimento padrão é um espaço (`pad =" "`) e, consequentemente, a sequência retornada estará ou alinhada à esquerda (`side =" left "`), alinhada à direita (`side =" right "`), ou
a ambos (`side =" both "`). 
...
A função `str_wrap ()` é equivalente a `strwrap ()` que pode ser usada para dar um 
quebrar uma string a fim de formatar parágrafos. A idéia de _wrap_ (equivalente aqui a quebrar) uma string (longa) é primeiramente, dividi-la em parágrafos de acordo com o "comprimento" fornecido e, em seguida, adicionar a indentação especificada em cada linha (primeira linha com `recuo`, seguindo linhas com `exdent`). O uso padrão tem o seguinte formato:
...
Agora, digamos que se deseja exibir o texto de `uma_citacao` dentro uma especificação prévia de comprimento da linha (por exemplo, cumprimento 3). Você pode fazer isso aplicando `str_wrap ()` e definindo o argumento `width = 30`
...
adicionar alguma indentação. Veja como você pode recuar a primeira linha, bem como as
linhas seguintes:
...
Uma das tarefas típicas do processamento de *strings* é a de analisar um texto em
palavras singularizadas. Normalmente, você termina com palavras que têm espaços em branco, chamados _whitespaces_, em qualquer uma das extremidade da palavra. Nessa situação, você pode usar a função `str_trim ()`  para remover qualquer número de espaços em branco nas extremidades de uma *string*. Seu uso requer apenas dois argumentos:
...
o `lado` no qual o espaço em branco será removido.

Considere o seguinte vetor de *strings*, alguns dos quais têm espaços em branco ou
à esquerda, à direita ou a ambos os lados. Segue o que  `str_trim ()` fará
...
Terminamos esse tutorial descrevendo a função `word()` que é usada para extrair palavras de uma sentença:

```
word(string, start = 1L, end = start, sep = fixed(" "))
```

A maneira como você usa  `word()` é passar uma `string`, juntamente com a posição
`start` da primeira palavra a ser extraída e uma  posição `end` da última palavra a ser extraída. Por padrão, o separador `sep` usado entre as palavras é um espaço único.
...
`stringr` tem mais funções que serão explicadas no segundo tutorial 
